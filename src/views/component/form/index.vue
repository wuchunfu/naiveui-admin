  ...
  <div>
    ...
    <n-card :content-style="{margin:0}">
      <BaseForm
          ...
          :label-align="formSetting.labelAlign"
          :items="formFiledList"
          :label-placement="formSetting.labelPlacement"
          :inline="formSetting.inline"
          :grid-props="{cols:formSetting.cols}"
          @submit="onSubmit">
      </BaseForm>
    </n-card>
  </div>
...
const formFiledList: Array<BaseFormItemProps> = [
  { field: 'menuName', label: '菜单名称', filedType: 'string', isSearch: false, required: true },
  { field: 'routeName', label: '路由key全局唯一', filedType: 'string', isSearch: false, required: true },
  { field: 'order', label: '显示顺序', filedType: 'number', isSearch: false, required: true },
  { field: 'isIframe', label: '是否为网页', filedType: 'switch', isSearch: false, required: true },
  { field: 'routePath', label: '请求路径', filedType: 'string', isSearch: false, required: true },
  ...
  { field: 'isHide', label: '是否显示', filedType: 'switch', isSearch: false, required: true },
  { field: 'permissions', label: '权限标识', filedType: 'string', isSearch: false, required: true },
  { field: 'menuType', label: '菜单类型', filedType: 'string', isSearch: false, required: true },
